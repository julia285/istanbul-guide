...
export async function createCafe(formData: FormData): Promise<void> {
  ...
  if (!nameEn || !nameTr || !descriptionEn || !descriptionTr) {
    redirect("/admin/cafes/new?error=missing-fields");
  }
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      redirect("/admin/cafes/new?error=duplicate-slug");
    }
    throw error;
  }

  revalidatePath("/[locale]/admin/cafes", "page");
  revalidatePath("/[locale]/cafes", "page");
  redirect("/admin/cafes");
}

export async function setCafeStatus(placeId: string, status: ContentStatus): Promise<void> {
