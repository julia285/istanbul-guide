...
const STYLE_TAGS = [
  { slug: "specialty-coffee", label: "Specialty coffee" },
  { slug: "roastery", label: "Roastery" },
  { slug: "hidden-gem", label: "Hidden gem" },
  { slug: "tourist-friendly", label: "Tourist-friendly" },
...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  "duplicate-slug": "A cafe with that name (in that language) already exists.",
};
...
export default async function NewCafePage({
  ...
  return (
    <div>
      <Link href="/admin/cafes" className="text-sm font-medium text-(--color-teal-700) hover:underline">
        ← Cafes
      </Link>
      <h1 className="font-display mt-3 text-2xl font-semibold text-(--color-teal-900)">Add a cafe</h1>
      ...
      <form action={createCafe} className="mt-6 max-w-2xl space-y-6">
        ...
        <label className="block">
          ...
          <input name="addressText" className={inputClass} placeholder="e.g. Caferağa Mah., Badem Altı Sk. No:21/B, Kadıköy" />
        </label>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <label>
            <span className={labelClass}>Cafe website</span>
