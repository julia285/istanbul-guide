...
import { setCafeStatus } from "./actions";
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-semibold text-(--color-teal-900)">Cafes</h1>
          <p className="mt-1 text-sm text-(--color-ink)/50">
            Everything published (or drafted) on the cafes page.
          </p>
        </div>
        <Link
      ...
      <div className="mt-6 space-y-3">
        {places.map((place) => {
          ...
          const toggleAction = setCafeStatus.bind(null, place.id, toggleStatus);
          return (
            <div
              key={place.id}
